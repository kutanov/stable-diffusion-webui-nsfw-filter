...
def censor_batch(x, safety_checker_adj: float):
    predictions = [onnx_model.predict(x_sample) for x_sample in x]
    index = 0
    for p in predictions:
        try:
            safety = filter(lambda predicate: p['predictions'] predicate.label == 'Safety')['confidence']
            naked = filter(lambda predicate: p['predictions'] predicate.label == 'Naked')['confidence']
